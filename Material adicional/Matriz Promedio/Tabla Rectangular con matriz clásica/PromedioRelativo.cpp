...
private:
	...
	// PRE: filas_utilizadas >= 0
	// PRE: col_utilizadas >= 0
...
public:
	...
	TablaRectangularReales PromedioRelativo (void)
	{
		...
		// Copia de la diagonal principal
		
		for (int n=0; n<filas_utilizadas; n++) 
			nueva.matriz_privada[n][n] = matriz_privada[n][n];
			
		// Calculamos los elementos del triángulo superior
		// (los del inferior se copian a partir del superior)
				
		for (int f=0; f<filas_utilizadas-1; f++) 

			for (int c=f+1; c<cols_utilizadas; c++) {
		
				double suma = 0.0;
		
				for (int casilla=c; casilla<cols_utilizadas; casilla++) 
					suma += matriz_privada[f][casilla];	
				
				nueva.matriz_privada[f][c] = suma/(cols_utilizadas-c);
		
				// Copiar el valor calculado en el elemento simétrico 
				nueva.matriz_privada[c][f] = nueva.matriz_privada[f][c];				
			}
			
			nueva.filas_utilizadas = filas_utilizadas;
